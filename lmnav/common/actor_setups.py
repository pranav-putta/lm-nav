...
import torch
from lmnav.emb_transfer.karmesh_eai_policy import KarmeshEAIPolicy
...
def setup_eai_teacher(config, env_spec):
    ...
    ckpt_dict = torch.load(config.generator.policy.load_artifact.dirpath, map_location='cpu')
...
@registry.register_fn('policies.karmesh_eai_policy')
def setup_karmesh_eai_teacher(config, env_spec):
    print("setting up eai teacher...")
    device = torch.device(config.exp.device)
    obs_transforms = get_active_obs_transforms(config)
    env_spec.observation_space = apply_obs_transforms_obs_space(
    ...
    teacher = KarmeshEAIPolicy.hardcoded(KarmeshEAIPolicy, obs_space, action_space)
