...
from lmnav.emb_transfer.embodiment_navmesh_util import set_pointnav_sim_agent, get_navmesh_name
...
@registry.register_task(name="ERNav-v0")
class ERNavTask(NavigationTask):
    def __init__(
    ...
    def reset(self, episode: EmbodimentEpisode):
        set_pointnav_sim_agent(self._sim, episode, self._config.AGENT_CONFIGS_FILE,
...
@registry.register_task(name='ERNavCorruptCameraTask')
class ERNavCorruptCameraTask(ERNavTask):

    def __init__(self, config: Config, sim: habitat.sims.habitat_simulator.habitat_simulator.HabitatSim,
                 dataset: Optional[Dataset] = None):
        super().__init__(config, sim, dataset)

        sim.agents[0]._sensors['depth'].noise_model = 'GaussianNoiseModel'
        sim.agents[0]._sensors['depth'].noise_model_kwargs = dict()
